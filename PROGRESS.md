...
## 筐改成「真实文件夹 ＋ 把文件移动进去」＋ 版本 3.2.1 → 3.3.0（2026-09-12）

领导原话：「改变储存的逻辑，文件夹里的文件不依赖于电脑桌面上。我现在的情况是新拖进去的文件如果电脑上删除的话就不能打开了」。

查下来的因果：筐里存的只是**路径**（当初定的规矩：只登记、绝不移动文件），所以原文件一删，筐里那条就成了死引用，双击自然打不开。

**三个决定都是领导拍的**（这次改动会真的动磁盘上的文件，不能替领导定）：

| 问题 | 选择 |
|---|---|
| 拖进筐的文件怎么进筐 | **移动**进筐，原位置不再保留 |
| 筐的文件放哪儿 | **`E:\文件筐\<筐名>\`**（和他们已有的文件目录同盘） |
| 原有 37 个条目 | **也一起收进筐**（启动时归位） |

### 新的边界（README 顶部那段承诺已按此改写）

- **每个筐 = 磁盘上一个真实文件夹**：加进来的文件（或整个文件夹）移动进去，从此归筐所有 ——
  原位置删掉、挪走，筐里照样打得开。
- **被 Dock 或别的筐引用着的文件只复制、不移动**：搬走会把那边弄坏（Dock 上的图标会变失效、
  别的筐会指向空）。实盘上他们的快捷方式大多同时被 Dock 收录，所以这批是复制进筐的。
- **移动的实现**：同盘用 `rename`（原子操作，不涉及删除）；跨盘先复制 → **校验**（逐层比名字与
  大小）→ 通过了才删源文件；校验不过就删掉半成品、保留源文件并如实报错。源文件被占用（比如
  正在运行的程序）时降级成"复制成功、原件留着"并说明。
- **永不删除用户的文件**：全仓唯一一处 `rmSync/unlink` 就在"移动的第二步"，且必然在移动路径上。
  从筐里移出、删除筐都只动清单（设置面板的「从筐里移出（不删文件）」文案同步改了）。
- 落盘从「设置文件 ＋ 图标缓存」两处变成**三处**（多了筐的文件夹），README 里写明了。

### 代码

- 新增 `src/main/basketfiles.js`：筐文件夹命名（非法字符/保留名/限长）、重名顺延（`名字 (2)`）、
  「已经在筐里 / 源文件没了 / 被别处引用 → 复制 / 可以搬」的决策、树指纹与复制校验、`moveInto`
  的落地实现。纯函数部分有 8 条单测（tests/basketfiles.test.js）。
- 新设置 `basket_dir`（默认 `E:\文件筐`，必须绝对路径，坏了回默认）；筐模型加 `dir` 字段；
  设置面板新增「筐的文件目录」（文本框 ＋「浏览…」，只影响之后新建的筐）。`dir:pick` 支持
  `purpose='baskets'` 复用同一个目录选择框。
- 四个入口（拖进弹窗 / 拖到 Dock 图标 / 弹窗「＋」/ 设置面板勾选）统一走
  `addPathsToBasket()`：先搬进筐目录、再把**筐里那份**登记进清单；搬不动的原样登记原路径并
  在日志与弹窗状态里说明（「N 项没能移进去（原文件留在原处）」）。
- **老条目归位**：启动时跑一遍，幂等（已在筐目录里的直接跳过），上次没搬成的下次会重试；
  源文件已经不在的保持原样（页面上照旧显示成缺失），绝不删登记。
- 候选清单 `basketCandidates` 改成**也按文件名**判断"是否已在这个筐里"（文件搬进筐后路径变了，
  只按路径比会显示成没加过、再勾一次又会多复制一份），并带回筐里那份的 `itemPath` 供取消勾选。

### 验收

- `npm test` → **116 passed / 0 fail**（新增 8 条）。
- **预演**（隔离实例 ＋ 领导配置的副本 ＋ 临时源目录里放三个真文件的小副本）：
  `移入 1、复制 2、没搬成 0` —— 一个文本文件被移进临时筐目录（源目录里已没有它），
  两个 .lnk 因为 Dock 自动收录也在用而复制（原件留在源目录），那条故意写错的不存在条目原样保留；
  空筐的目录也建好了。
- **预演里真拖了一次**：从资源管理器把桌面上的测试文件拖进筐弹窗 →
  `[basket] 加入（来源 popup.html）：登记 1 条（移入 1、复制 0、失败 0）`，
  桌面上的那个文件**不见了**、文件躺在那个筐自己的目录里 —— 这正是领导要的语义。
- **实盘**（真实配置与文件）：重启后 `[basket] 老条目归位：移入 34、复制 3、没搬成 0`；
  三个筐 37 项**零缺失**、全部指向 `E:\文件筐\<筐名>\`；Dock 的 12 个条目**全部存活**
  （来源目录里留下的正好是 Dock 在用的那批），来源目录从 48 项收到 14 项。
- 没能实机触发的两条：① 跨盘的"复制 → 校验 → 删源"这条路径（领导的文件都在 E 盘，
  走的是同盘 rename）；② 源文件被占用时的降级分支。两条都有单测覆盖的决策逻辑与代码注释说明。

### 留给下次

- **PS 5.1 读无 BOM 的 UTF-8 脚本会把中文当 ANSI**：种子脚本里的 `'D:\shortcuts'` 这类路径直接变成乱码、
  复制全失败。写 .ps1 要么带 BOM（`utf-8-sig`），要么把中文路径从命令行传参。
- **PowerShell 变量名不分大小写**：`foreach ($d in ...)` 会把 `$D` 覆盖掉，路径就跑到别处去了
  （这次差点把记录写到别的目录去）。脚本里别用只差大小写的变量名。
- 验证用的临时目录、脚本、测试文件都清干净了；领导的桌面/来源目录/配置没有被这次验证弄脏
  （桌面那个 html 是领导自己 10:31 删到回收站的，与本次改动无关）。

## 筐弹窗的键鼠操作 ＋ 删除即消失 ＋ 移出到桌面 ＋ 版本 3.3.0 → 3.4.0（2026-09-13）

领导原话（附截图，筐里一条灰色的「新建 BMP 图像.bmp」被圈出来）：①「文件筐里面的文章无法移出到
外面来」②「在文件管理器里删除后，不应该在文件筐里面显示灰色，而是直接消失」③「在文件筐中，
win 的自带右键以及快捷键不能直接使用」。

### 三件事的落点

1. **移出到桌面**：右键「移出到桌面」——真把文件从筐目录搬回桌面（重名自动加 (2)），清单同步去掉。
   走的是 placeInto 的 move 通道（同盘 rename / 跨盘复制校验），与"加进筐"互为逆操作。
   原来的「从筐里移出（不删文件）」从弹窗菜单里拿掉了（设置面板的取消勾选仍是只取消登记，
   文件留在筐文件夹——提示文案写明）。
2. **删除即消失**：`basketView` 现在会把"文件已不存在"的登记**直接清掉**（纯清单操作，磁盘上
   本来就没这文件了），页面上不再有灰色残影。两个保险：只在该筐的 `dir` 目录本身 accessible 时
   才清（盘不在/被占用时整筐不动，免得一次误判清光）；弹窗开着时每 2 秒取一次视图，变了就重画，
   所以在资源管理器里删掉的几秒内弹窗自动跟上（弹窗被别的窗口完全盖住时 Electron 判为不可见、
   轮询暂停，回到可见 2 秒内补上）。顺带把"加东西时源文件已不存在"改为**不登记**死条目（以前会
   登记成灰色）。
3. **键鼠操作**：这种自绘无边框窗口挂不了 Windows 原生 shell 右键菜单（要系统级 shell 扩展），
   把常用的一套补齐到自绘菜单 ＋ 快捷键：
   - 选中：单击选中、Ctrl 点选、Shift 连选、Ctrl+A 全选（操作作用于整个选区）；
   - `Ctrl+C` 复制真文件进系统剪贴板（资源管理器里能直接粘贴出来）；
   - `Ctrl+V` 把剪贴板里的文件收进筐——**剪贴板带"剪切"意图就移动、"复制"意图就复制**（读
     Preferred DropEffect），Dock/别的筐引用着的文件保护规则两种模式都生效；
   - `Delete` 删除 → `shell.trashItem` 进系统回收站（可恢复）；
   - `F2` 行内重命名 → 改的是筐目录里的**真文件名**（同盘 rename，登记同步）。
   - Electron 的 clipboard 模块放不了资源管理器认的文件列表（CF_HDROP），复制/粘贴读两步交给
     一次性 PowerShell：复制用 `Set-Clipboard -Path`；粘贴读 FileDropList ＋ Preferred DropEffect，
     **路径走 base64 行式协议**传回——控制台输出编码（GBK）在 `-EncodedCommand` ＋ 隐藏窗口下
     靠不住，连 `[Console]::OutputEncoding = UTF8` 都会抛"句柄无效"把脚本打死（$ErrorActionPreference
     = Stop），与 shellIcons 的行式协议是同一个坑。

### 验收

- `npm test` → **118 passed / 0 fail**（新增 sanitizeFileName、dropMissing 共 4 条断言组）。
- 真机（隔离实例：一个筐放 甲/乙/丙 三个真文件，PowerShell 真实光标 ＋ CUA 键盘逐项演练）：
  - **删除即消失**：在资源管理器里删掉 甲.txt → 重开弹窗直接没有它（日志 `清掉 1 条已不存在的
    登记`，配置 3 → 2）；顺带验证了自动清理对"路径失效"的正确性——种子脚本没带 BOM 把路径写成
    乱码时，弹窗按新语义清空清单而不是显示三条灰色 ✅（这就是领导圈的灰色 BMP 的解法）；
  - **F2 改名**：行内输入框 → Enter → 磁盘真改名 ＋ 登记同步 ✅（自动化键入被系统中文输入法搅出
    乱码名，是键入工具的问题，产品链路本身正确）；
  - **移出到桌面**：右键 → 文件出现在桌面、筐目录与清单同步 ✅；
  - **Ctrl+C / Ctrl+V**：资源管理器复制 → 弹窗 Ctrl+V → 复制语义（桌面原件保留）✅；
    弹窗 Ctrl+C → 剪贴板里是筐里的真文件路径（资源管理器可粘贴）✅；
  - **Delete 键**：进回收站 ✅。
- 实盘：重启到 3.4.0，三个筐 38 项全部存活、零失效。
- 没能实机触发的：剪贴板里带"剪切"意图的移动分支（自动化里没造出 DropEffect=move 的剪贴板，
  代码走的是与移动相同的 placeInto 通道，保护规则有单测）。

### 留给下次

- **PS 5.1 在 `-EncodedCommand` ＋ 隐藏窗口下，`[Console]::OutputEncoding = UTF8` 会抛"句柄无效"**
  （`$ErrorActionPreference='Stop'` 时直接打死脚本、stdout 全空）——和 PowerShell 读无 BOM 脚本
  乱码是两个独立的坑。凡是往回传中文/二进制，一律 base64 行式协议。
- **自动化键入会被中文输入法搅乱**（字母被吞、句点变「・」）：键鼠预演尽量只用特殊键（F2/Del/^c）
  和鼠标；非要打字用 CUA 的 type/key（这次 WScript.SendKeys 的字符不可靠）。
- 弹窗被完全盖住时 Electron 判 hidden，渲染层轮询要自觉停；"回到可见 2 秒内补刷"就是为此设计。

## 全项目走查 ＋ 重命名修复 ＋ 拖拽移出 ＋ 重写 README ＋ 版本 3.3.0 → 3.5.0（2026-09-13）

领导原话：「请你检查整个项目，确保没有bug，重写readme，同时不上传我的任何数据到github上，提交github」；
中途补了两条：「等下，重命名失效」「我希望可以实现鼠标直接将文件移进移出的功能」。

### 走查修掉的问题

- **重命名失效（领导实测，修了两轮才中）**：第一轮猜"右键菜单关掉后键盘焦点没回来"，加了
  `popup:focus` 把弹窗推前台 + 输入法组词期 Enter 防护 + 提示顺序修正——**领导实测依旧失效**。
  第二轮加诊断日志在隔离实例复现（F2 与右键菜单两条路径都跑），日志说明真凶：
  `startRename` 原来是"**先挂输入框、再去抢窗口焦点**"，抢焦点这一下让刚创建的输入框瞬间失焦，
  而 `blur` 被判定为"用户放弃" → 改名被静默取消（现象就是"点了没反应"）。
  改成"**先 popupFocus 等窗口拿到前台，再挂输入框并聚焦**"，外加"刚打开 500ms 内的失焦不算放弃"
  （窗口前后台切换期的抖动）。两轮验证：F2 路径 `[basket] 重命名：two.txt → two` ✓、
  右键菜单路径 `two → three` ✓，两条都不再出现"取消"。
  教训：**行内编辑器的挂载必须发生在窗口拿到焦点之后**，别让"刚挂上就失焦"被当成用户行为。
- **placeInto 的清理竞态**：复制失败清理"半成品"时，若目标位刚被并发操作抢先占用（EEXIST），
  原实现会把**别人的文件**删掉。现在 EEXIST 不清理、如实报错。
- **snapshotTree 跟随符号链接**：目录环（联接点指回上级）会无限递归。改用 lstat，
  链接按链接本身记录——两端对称，校验依然有效。
- **改名失败/取消的提示被重画覆盖**：render 会把状态行写成"N 项"，提示必须放在重画之后。
- **右键菜单开着时视图自动重画**：菜单关掉后可能把改名输入框挂到已脱节的格子上（用户看不见）。
  挂之前检查 `cell.isConnected`，脱节就先重画。

### 新增：鼠标拖拽移进/移出

- **拖进来 = 移动进筐**（3.4.0 已有，回归通过）。
- **拖出去 = 从筐里拖到桌面/资源管理器**：格子 `dragstart` → 主进程 `webContents.startDrag`
  （图标用 shellIcons 取的真图标，兜底 assets/icon.ico）。**语义是复制一份到落点、筐里保留**——
  startDrag 拿不到"拖到哪了/放没放下"的回执，做不到拖出去就删筐里那份；真要移出用右键
  「移出到桌面」。拖完弹窗状态行会说明这一点，README 常见问题也写了。

### 其他

- README 整体重写：开篇就讲清"文件怎么被对待"（移动进筐 / 只复制不移动 / 回收站 / 跨盘校验），
  键鼠一览表、拖拽两个方向的语义、常见问题补两条（拖出为何是复制、删掉的还能不能找回）。
- 个人数据终扫：全仓 grep 用户名/筐名/文件名/来源目录等标识，零残留；桌面快照与含文件名的
  截图此前已移出仓库并被 .gitignore 排除。

### 验收

- `npm test` → **118 passed / 0 fail**。
- 真机（隔离实例）：右键 → 重命名（焦点修复后输入框能正常打字提交）、拖入回归待领导实试
  （本次验证进行到一半，领导开始使用机器，自动化鼠标立即停止——不能打扰）。
- 实盘重启到 3.5.0。

## Dock 最左侧加开始菜单图标 ＋ 版本 3.5.0 → 3.6.0（2026-09-13）

领导原话：「帮我在dock的最左侧添加windows图标。点击就能弹出windows的系统弹窗。」

### 落点

- 作为第 4 个系统虚拟项加入 `specials.js`（id `windows`，kind `start`，label「开始菜单」），
  默认配置与设置面板勾选都排在最前 → Dock 最左侧。
- 图标没有 shell 来源（开始按钮不是文件），`special:icon` 为它内置一张同款配色 SVG
  （Windows 四格窗标，#0078D4）。
- 点击不走 shell 打开：注入一次 **Win 键**（`keybd_event(0x5B)` 按下＋抬起）弹出系统开始菜单。
  keybd_event 是全局注入，不需要窗口焦点，也不会打断当前输入。为什么不走 Ctrl+Esc 的
  SendKeys：COM SendKeys 的字符/组合在这台机器上不可靠（输入法会搅），键位注入更稳。
- 老配置以用户写的 `dock_specials` 为准，不硬塞（设置面板可随时勾选/撤下）。

### 验收

- `npm test` → **118 passed / 0 fail**（更新 dock_specials 默认值的断言）。
- 真机（隔离实例，自动收起 Dock）：Dock 最左侧出现四格蓝窗图标；点击后前台窗口变为
  「搜索」（Win11 开始界面宿主），截图确认开始菜单弹出（已固定/全部应用/账户/电源齐全）。
- 实盘配置已加 `windows` 到 `dock_specials` 首位并重启实例。

## Store 应用快捷方式的图标自己取 ＋ 版本 3.7.0 → 3.7.1（2026-09-16）

领导原话：「帮我修复这个问题，确保以后不会遇到这个问题」。

### 现象与根因

- 现象：Store（MSIX）应用的快捷方式在 Dock 与筐里都是通用「白纸」图标。本机三个（Watt Toolkit /
  Fluent Reader / Snipaste）全中，而 `E:\快捷方式` 里其余快捷方式（微信 / Steam / QQ音乐 / Edge…）正常。
- 根因不在本仓库：这类 `.lnk` 里没有文件目标，目标是一串 AUMID（`<包族名>!<应用 id>`），
  而 **shell 渲染不出它们的图标** —— `SHGetFileInfo` 对 `.lnk` 和对 `shell:AppsFolder\<AUMID>`
  都回通用白纸，缩略图接口（`IShellItemImageFactory`）同样只给白纸小图（计算器这类正经注册了图标的
  UWP 应用则正常）。shellIcons 一直如实转述 shell 的答案，所以白纸是「照实画」，不是画错。
- 另一个坑：exe 换了内嵌图标后，**shell 会按路径把旧图标缓存住**（改 mtime、发 `SHChangeNotify`
  都不失效，只能清 `iconcache_*.db` 并重启资源管理器）。这条与代码无关，但排查时会撞上。

### 落点

- 新增第四种请求种类 `a<AUMID>`。`.lnk` 的目标 ID 列表里能扫出 AUMID（纯函数 `aumidFromLink`，
  0/1 两个相位扫 UTF-16；认不出就退回原路），命中就改读**应用包里的 logo 素材** —— 开始菜单磁贴
  用的是同一套图。取不到（包已卸载、素材被裁）自动退回原来的 shell 路线，不会比从前差。
- 素材挑选按**像素尺寸**、排序键全显式、末位用路径：只认方形 PNG，跳过 lightunplated / white /
  black / contrast 与 Badge / Wide / SplashScreen；优先 unplated 家族；再取「不小于目标尺寸里最小的
  那张」，都不够大才取最大的。**这里踩过坑**：原先用文件名正则挑 targetsize 数字，而正则写在 JS
  模板字符串里 —— `'targetsize-(\d+)'` 的 `\d` 被 JS 当转义吃掉，PS 收到的是 `targetsize-(d+)`，
  永不匹配 → 所有 unplated 素材并列同分 → 排序退化成目录枚举顺序，实机上恰好选中 16×16 那张，
  放大到 128 就是一团糊。现在不依赖文件名猜测，且末位排序键是路径，彻底去掉枚举顺序的影响。
- 入口只有一处：`iconDataUrl()` 里先判 AUMID 再退回文件路线，Dock（128px）、筐弹窗（48px）、预取、
  翻目录全都自动受益；候选表 `iconSources` 不用动。

### 验收

- `npm test` → **124 passed / 0 fail**（新增 6 条：AUMID 认得出来、UTF-16 奇数对齐、普通文件路径 /
  MSI 通告式 / 非 `.lnk` 不误认、AUMID 后面跟别的令牌只取 AUMID；请求编码补 `a` 前缀断言）。
- 真机（隔离实例）：三个 Store 应用走 `a<AUMID>` 路线，48px 与 128px 都拿到应用包原图；
  watt@128 与「素材正确缩放」的参考图逐像素一致（MAE=0.00），fluent@128 与素材缩放 MAE=2.3。
  重启实例后应用自己写入的图标缓存即为这些图。
- 回归 10 条：包已卸载的旧 Edge（退回 shell 路线拿到真 Edge 图标）、微信 / Steam / QQ音乐
  （MSI 通告式）、两个 exe、一个文件夹，全部与从前一致。
- 本机那三个 Store 快捷方式另外补了显式 IconLocation（资源管理器里也不再是白纸）——那是数据改动，
  不进仓库。
