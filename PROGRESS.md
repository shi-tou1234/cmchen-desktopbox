...
- **重命名失效（领导实测）**：右键菜单是独立小窗口，点「重命名」后键盘焦点不一定回到弹窗——
  输入框开了但打字/回车全落空。修法：新增 `popup:focus` IPC，打开行内改名框前把弹窗推到前台聚焦；
  输入框加 `isComposing` 防护（中文输入法组词期间的 Enter 是"确认候选"，不是提交）；
  提交顺序也修正为"先改名落盘 → 再重画 → 后报结果"（原来状态行会被 render 的"N 项"盖掉）。
