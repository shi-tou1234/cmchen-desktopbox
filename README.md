...
- **底部 Dock**：**只在桌面上显示**，不压在其他程序上面。
  ...
  - **流畅度**：图标在窗口出现**之前**就算好塞进载荷（不在展开动画播到一半时才开始取图标/解码图片）；后台预热筐里条目的图标；**弹窗窗口复用**——第一次打开才起渲染进程，之后都是热窗口（实测点击到显示 136ms → 3ms）；**等页面画出第一帧再显示窗口**（否则磨砂材质会先闪一个空白矩形），显示之后才播展开动画；完全透明模式不再做多余的整窗淡入（页面自己已经在淡）；翻目录不等图标（后台预热，第二次秒开）；Dock 的逐帧循环在静止时会停掉，不空跑 60fps。
