...
|---|---|---|---|---|
| **磨砂玻璃**（默认，与 token 的 `systemGlass` 默认一致） | `transparent: false` + `backgroundMaterial: 'acrylic'` | 0.86 | **0.09** | 背后内容被糊开，是真正的磨砂 |
| **完全透明** | `transparent: true`，不挂材质 | **0.92** | 0.27 | 桌面完全透出来，只有一道细边框和一层文字阴影 |
