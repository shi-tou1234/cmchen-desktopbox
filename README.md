...
1. **做不到"压在普通窗口之下、Win+D 后仍与桌面图标同时可见"**：现代 Windows 的壁纸层就在 z 序最底部，`HWND_BOTTOM` 会把窗口压到壁纸下面；挂 Progman/WorkerW 子窗口则 DWM 从不合成。2026-09-11 又补测了四种 `SetWindowPos` 插入点（插到 Progman 上方那个窗口之后、`HWND_BOTTOM`、插到前台窗口之后、隐藏一瞬问出背后窗口再插到它后面），**全部会让窗口掉到桌面层之下**（`IsWindowVisible` 为真但看不见）。所以 Dock 不做"桌面层"，改成**贴底边滑出、鼠标离开自动收起**：不挡任何窗口，需要时碰一下底边。
2. **Dock 与自动隐藏的任务栏在底部会视觉重叠**：Dock 贴屏幕底边，任务栏自动隐藏时两者会被同一次"鼠标到底边"动作同时唤起（Dock 会盖在任务栏之上）。想彻底避开就得改用 AppBar 预留空间，但那样桌面可用高度会永久减少约 40 像素。
