...
- **往筐里添加东西**（新建文件夹后常问这个）：四种方式，一次都能加很多——① **设置面板里勾选**：选中一个筐，下面就是桌面上所有条目的清单，勾上即加入、取消即移出（不用开对话框，几十个条目在一个列表里点）；② 弹窗左上角的「＋」选文件（对话框里按住 Ctrl / Shift 多选，默认停在桌面）；③ 把**一批文件直接拖进弹窗**；④ 把一批文件**拖到 Dock 上那个文件夹图标上**（图标会亮一圈，松手时名字那一行闪一下「＋N」当回执）。注意：Dock 上的**快捷方式**归 Dock 管，筐只登记路径——加进筐里不会移动/复制磁盘上的文件。设置面板里还有「添加文件…／添加文件夹…」两个入口，用来加不在桌面上的东西。
- **底部 Dock**：**只在桌面上显示**，不压在其他程序上面。
  ...
  - 图标取的是**不带 Windows 小箭头**的原图（`.lnk` 交给 Windows shell 解析后取系统图标列表里的原图，叠加的小箭头是 shell 绘制视图时才加的，不在图里）；取图时还会**按内容裁掉空白边再缩放到满格**——有些 `.ico` 的 48/256 槽位里只在左上角画了一小块（只带了 16×16 资源的那些），原样画出来就是"图标偏小"。「此电脑 / 回收站」两个系统图标也支持右键移除。
  ...
  - **流畅度**：图标在窗口出现**之前**就算好塞进载荷（不在展开动画播到一半时才开始取图标/解码图片）；后台预热筐里条目的图标；**弹窗窗口复用**——第一次打开才起渲染进程，之后都是热窗口（实测点击到显示 136ms → 3ms）；**等页面把新内容画出来再显示窗口**（否则磨砂材质会先亮一块空白矩形、复用时会闪出上一次的内容）；开合的淡入淡出**全部由页面用 CSS 做，窗口自己不动透明度**——给挂系统材质的窗口 `setOpacity` 会让材质底闪一下（这就是"开和关都闪一闪"的来源）；磨砂模式下的开合改成"轻微缩放 + 淡"，避免内容缩得太小和整块材质底对不上；翻目录不等图标（后台预热，第二次秒开）；Dock 的逐帧循环在静止时会停掉，不空跑 60fps。
...
- **设置面板**：外观（磨砂模式、弹窗图标大小）、Dock（显示开关、常驻/自动收起、图标大小、悬停放大程度、离屏幕底边的距离）、快捷方式清单（勾选 / 改名）、文件夹管理（改名 / **桌面条目勾选清单（勾上就进筐）** / 添加文件 / 添加文件夹 / 清理失效项 / 新建 / 删除）、开机自启、恢复默认。
